...
- 首次安装模块后，请先于 `/data/adb/box_bll/clash/config.yaml` 添加你的订阅地址，随后需手动重启设备一次
- 可能因网络原因不会自动下载完全部 **规则**/**订阅**，请至面板手动刷新一下
  - 如上述失败，并确保你的网络环境正常
- Web App：
  - 仅为图形辅助工具，用于便携浏览及管理后台路由数据，并无其它多余用途
  - 如果遇到**请求资源失败正在加载本地面板**，或者代理组的图标无法显示，您可以尝试 关闭/开启网络，亦或者检查是否能访问上 `raw.githubusercontent.com`
